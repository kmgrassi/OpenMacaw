defmodule SymphonyElixir.Codex.AppServer do
  ...
  require Logger
  alias SymphonyElixir.{Codex.AppServer.Approvals, Codex.AppServer.Transport, Config, PathSafety}
  ...
  def run_turn(
    ...
    case Transport.start_turn(port, thread_id, prompt, issue, workspace, approval_policy, turn_sandbox_policy) do
      {:ok, turn_id} ->
  ...
  def interrupt_turn(%{port: port, thread_id: thread_id}, turn_id)
    ...
    case Transport.send_message(port, %{
  ...
  defp do_start_session(port, workspace, session_policies) do
    case Transport.send_initialize(port) do
      :ok -> Transport.start_thread(port, workspace, session_policies)
    ...
    case Jason.decode(payload_string) do
      ...
      {:ok, payload} ->
        emit_message(
          ...
          Transport.metadata_from_message(port, payload)
        )
      ...
      {:error, _reason} ->
        ...
        if protocol_message_candidate?(payload_string) do
          emit_message(
            ...
            Transport.metadata_from_message(port, %{raw: payload_string})
          )
        end
  ...
  defp handle_turn_method(
    ...
    metadata = Transport.metadata_from_message(port, payload)

    case Approvals.handle_request(
           port,
           method,
           payload,
           payload_string,
      ...
      :unhandled ->
        if Approvals.needs_input?(method, payload) do
          emit_message(
            on_message,
            :turn_input_required,
            %{payload: payload, raw: payload_string},
